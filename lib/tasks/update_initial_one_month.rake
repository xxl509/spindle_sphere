desc "update the initial status with one month blank"
task :update_initial_one_status => :environment do
	batch_index = 1
	Patient.where(initial_status: true).each do |p|
		p.initial_one_month = true
		p.save
	end
	Patient.where("initial_status is NULL").find_in_batches(batch_size:1000) do |b|
		puts batch_index
		batch_index += 1
		b.each do |p|
			pid = p.patient_id
