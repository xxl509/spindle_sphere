...
task :export_sex_data_old => :environment do
	require 'csv'
	...
	CSV.open("#{Rails.root}/lib/assets/sur_sex_6_11.csv", "wb") do |csv_object|
		...
		Patient.where('survival_date >0  and exposure_age is not null and exposure_age in (?)', (6..11).to_a).find_in_batches(batch_size: 1000).each do |g|
			g.each do |p|
				if p.sex == 'M'
					row = [p.patient_id]
					if p.ccs_661 >0
					...
					# if control_patient.ccs_661 == 1
					# 	c_row << 1
					# else
					# 	c_row << 0
					# end
					# c_row << control_patient.survival_date
					# c_row << exposure_age_group["#{p.exposure_age}mc"]
					# csv_object << row
				elsif p.sex == 'F'
					row = [p.patient_id]
					if p.ccs_661 >0 
					...
					if p.exposure_age.present?
						row << exposure_age_group["#{p.exposure_age}f"]
					else
					...
					# end
					# c_row << control_patient.survival_date
					# c_row << exposure_age_group["#{p.exposure_age}fc"]
					# csv_object << row
				end
			end
		end
	...
	CSV.open("#{Rails.root}/lib/assets/sur_sex_12_13.csv", "wb") do |csv_object|
		...
		csv_object << ["ID", "event", "date", "exposure_age_group"]
		Patient.where('survival_date >0 and exposure_age is not null and exposure_age in (?)', (12..13).to_a).find_in_batches(batch_size: 1000).each do |g|
			g.each do |p|
				if p.sex == 'M'
					row = [p.patient_id]
					if p.ccs_661 >0
	...
	CSV.open("#{Rails.root}/lib/assets/sur_sex_14_15.csv", "wb") do |csv_object|
		exposure_age_group = {'14m'=>0,'15m'=>0,
														'14f'=>1,'15f'=>1}
		csv_object << ["ID", "event", "date", "exposure_age_group"]
		Patient.where('survival_date >0 and exposure_age is not null and exposure_age in (?)', (14..15).to_a).find_in_batches(batch_size: 1000).each do |g|
			g.each do |p|
				if p.sex == 'M'
					row = [p.patient_id]
					if p.ccs_661 >0
	...
	CSV.open("#{Rails.root}/lib/assets/sur_sex_16_17.csv", "wb") do |csv_object|
		exposure_age_group = {'16m'=>0,'17m'=>0,
														'16f'=>1,'17f'=>1}
		csv_object << ["ID", "event", "date", "exposure_age_group"]
		Patient.where('survival_date >0 and exposure_age is not null and exposure_age in (?)', (16..17).to_a).find_in_batches(batch_size: 1000).each do |g|
			g.each do |p|
				if p.sex == 'M'
					row = [p.patient_id]
					if p.ccs_661 >0
